...
def main():    
    ...
    def move_linear(x, y, z, speed=0.5):
        ...
        # Use PTP planner for linear-like movement (more flexible than LIN)
        pose = Robpose()
        pose.x = float(tgt_x)
        pose.y = float(tgt_y)
        pose.z = float(tgt_z)
        ...
        result = robot.RobMove_EXECUTE("PTP", speed, pose)
        return result["Success"]
    ...
    while True:
        ...
        move_joints(-167.0, -142.0, -38.0, -90.0, 91.0, -344.0)

        print("3. Moving to Analice Approach ...")
        move_joints(-165.0, -123.0, -73.0, -74.0, 91.0, -342.0)
        time.sleep(2.0)
        ...
        move_joints(-167.0, -144.0, -49.0, -78.0, 91.0, -344.0, speed=0.25)
        time.sleep(3.0)
        gripper.CLOSE(20.0)
        
        print("6. Moving to loading Position ...")
        move_joints(-167.0, -103.0, -79.0, -79.0, 91.0, -340.0)

        print("7. Moving to Place Approach...")
        if shape == "box": # GREEN
            print("Target: GREEN Zone")
            move_joints(16.0, -143.0, -33.0, -71.0, 78.0, -150.0)
        elif shape == "cylinder": # RED
            print("Target: RED Zone")
            move_joints(-77.0, -160.0, -7.0, -87.0, 89.0, -176.0)
        elif shape == "prism": # BLUE
            print("Target: BLUE Zone")
            move_joints(-262.0, -147.0, -31.0, -78.0, 87.0, -167.0)
